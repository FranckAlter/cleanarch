...
using exam_sales_reporter_kata.Printer;
...
namespace exam_sales_reporter_kata.Cli
{
	public static class Program
    {
    	...
    	public static void Main(string[] args)
    	{
            ...
            Loger.display("=== Sales Viewer ===");
            //extract the command name from the args  
    		string command = args.Length > 0 ? args[0] : "unknown";  
    		string file = args.Length >= 2 ? args[1] : "/home/franck/Documents/Informatique/M2/archi/exam-sales-reporter-kata/ExamSalesReporterKata/Data/data.csv";
    		...
    		{  
    		 //get all the lines without the header in the first line  
                 ...
                 Loger.display($"+{new String('-',45)}+");
                 Loger.display($"| {" Number of sales".PadLeft(30)} | {number1.ToString().PadLeft(10)} |");
                 Loger.display($"| {" Number of clients".PadLeft(30)} | {clients.Count.ToString().PadLeft(10)} |");
                 Loger.display($"| {" Total items sold".PadLeft(30)} | {number2.ToString().PadLeft(10)} |");
                 Loger.display($"| {" Total sales amount".PadLeft(30)} | {Math.Round(number3,2).ToString().PadLeft(10)} |");
                 Loger.display($"| {" Average amount/sale".PadLeft(30)} | {number4.ToString().PadLeft(10)} |");
                 Loger.display($"| {" Average item price".PadLeft(30)} | {number5.ToString().PadLeft(10)} |");
                 Loger.display($"+{new String('-',45)}+");
    		}  
    		else  
    		{  
	            Loger.display("[ERR] your command is not valid ");  
                 Loger.display("Help: ");  
                 Loger.display("    - [print]  : show the content of our commerce records in data.csv");  
                 Loger.display("    - [report] : show a summary from data.csv records ");  
    		}
    	}
    }
}
